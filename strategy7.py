...
def init(context):
    ...
    context.window_short = 120
    context.window_long = 250
    context.update_stocks = []
    context.short = 20
    context.long = 120
    context.SD = 0.06
    context.longest_history = context.long
...
def handle_bar(context, bar_dict):
    ...
    logger.info(context.update_stocks)
    for stock in context.update_stocks:
        price_history = history_bars(stock, context.longest_history, '1d', 'close')

        # 计算短均线
        short_mean = np.mean(price_history[-context.short:])

        long_mean = np.mean(price_history[-context.long:])

        # 计算买入卖出信号
        flag = True if (short_mean - long_mean) > context.SD * long_mean else False
        if flag:
            # 买入
            logger.info("买入：" + stock)
            result = order_target_percent(stock, 0.1)
...
def task(context, bar_dict):
    temp = []
    result_long = calRPS(context.stocks, context.window_long)
    result_short = calRPS(context.stocks, context.window_short)

    for i in range(result_long.shape[0]):
        if result_long.rpsValue[i] >= 90:
            for index, stock in enumerate(list(result_short.code)):
                if result_long.code[i] == stock and result_short.rpsValue[index] >= 90:
                    temp.append(stock)
    context.update_stocks = temp
